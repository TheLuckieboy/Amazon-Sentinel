...
def generate_random_Date(BadgeActivation=False, BadgeDeactivation=False, LastUpdate=False, base_date=None, next_activation_date=None):
    ...
    if BadgeActivation:
        ...
        # Generate a random last update date between activation and deactivation
...
def generate_profile():
    ...
    for _ in range(badge_count):
        # Generate badge dates relative to a progressively older base date
        badge_activate = generate_random_Date(BadgeActivation=True, base_date=base_date)
        badge_deactivate = generate_random_Date(BadgeDeactivation=True, next_activation_date=next_activation_date)
        badge_lastupdate = generate_random_Date(LastUpdate=True)
        ...
        next_activation_date = badge_activate

        # Move the base date backward for the next badge
        base_date -= timedelta(days=random.randint(1, 365))  # Adjust by 1-365 days
