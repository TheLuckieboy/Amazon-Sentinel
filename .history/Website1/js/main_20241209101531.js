document.addEventListener("DOMContentLoaded", function() {
    ...
    function generateBadgeTableHTML(profileData) {
        ...
        // Build the table header with sorting functionality
        const headers = [
            { label: 'Badge ID', column: 'Badge_ID' },
            { label: 'Status', column: 'Badge_Status' },
            { label: 'Type', column: 'Badge_Type' },
            { label: 'Activate On', column: 'Badge_ActivateOn' },
            { label: 'Deactivate On', column: 'Badge_DeactivateOn' },
            { label: 'Last Updated', column: 'Badge_LastUpdatedUTC' },
            { label: 'Last Location Reader Name', column: 'Badge_LastLocationReaderName' },
            { label: 'Last Location Event Type', column: 'Badge_LastLocationEventType' }
        ];
    
        const headerHTML = headers.map((header) => {
            const isActive = sortState.column === header.column;
            const sortIcon = isActive
                ? `<svg viewBox="0 0 16 16"><path d="m8 11 4-6H4l4 6Z" class="${sortState.order === 'asc' ? 'filled' : ''}"></path></svg>`
                : '';
            return `
                <th class="awsui_header-cell_1spae_13mz6_145 awsui_header-cell-sortable_1spae_13mz6_212"
                    scope="col" onclick="sortBadges('${header.column}')">
                    <div class="awsui_header-cell-content_1spae_13mz6_272">
                        <div class="awsui_header-cell-text_1spae_13mz6_344">${header.label}</div>
                        <span class="awsui_sorting-icon_1spae_13mz6_258">${sortIcon}</span>
                    </div>
                </th>
            `;
        }).join('');
        ...
        const badgeRows = profileData.Badges.map((badge, index) => `
            <tr class="awsui_row_wih1l_1v07r_356" aria-rowindex="${index + 2}">
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_ID || ''}</td>
                <td class="awsui_body-cell_c6tup_et2x0_148">
                    <span class="awsui_badge-color-${badge.Badge_Status === 'Active' ? 'green' : 'blue'}">${badge.Badge_Status || ''}</span>
                </td>
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_Type || ''}</td>
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_ActivateOn || ''}</td>
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_DeactivateOn || ''}</td>
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_LastUpdatedUTC || ''}</td>
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_LastLocationReaderName || ''}</td>
                <td class="awsui_body-cell_c6tup_et2x0_148">${badge.Badge_LastLocationEventType || ''}</td>
            </tr>
        `).join('');
    
        // Combine the table header, rows, and footer
        return `
            <table class="awsui_table_wih1l_1v07r_198">
    ...
    // Function to sort badges by a specific column
    function sortBadges(column) {
        ...
        profileData.Badges.sort((a, b) => {
            const valueA = a[column] || '';
            const valueB = b[column] || '';

            // Handle sorting as numbers or strings
    ...
    document.getElementById('table-header-2466-1733517192643-2830').addEventListener('click', sortBadges);
});
