...
def generate_random_Date(BadgeActivation=False, BadgeDeactivation=False, LastUpdate=False, base_date=None, activation_date=None, next_activation_date=None):
    ...
    if BadgeActivation:
        ...
        ActivationDate_Time = ActivationDate.strftime("%Y-%m-%d %H:%M:%S")
        ...
        LastUpdateDate_Time = LastUpdateDate.strftime("%Y-%m-%d %H:%M:%S")
...
def generate_randdom_AccessLevelActivateOn():
    global ActivationDate_Time, LastUpdateDate_Time, AccessLevelActivateOn
    ...
    if choice == 1:
        ...
        result = datetime.strptime(ActivationDate_Time, "%Y-%m-%d %H:%M:%S")
        AccessLevelActivateOn = result + timedelta(days=1)
        return AccessLevelActivateOn.strftime("%Y-%m-%d %H:%M:%S")
    else:
        AccessLevelActivateOn = "PERMANENT"
        return AccessLevelActivateOn
    
def generate_randdom_AccessLevelDeactivateOn():
    global AccessLevelActivateOn, LastUpdateDate_Time
    AccessLevelDeactivateOn = AccessLevelActivateOn

    if AccessLevelDeactivateOn == LastUpdateDate_Time:
        return AccessLevelDeactivateOn
    elif AccessLevelDeactivateOn != "PERMANENT":
        result = datetime.strptime(AccessLevelDeactivateOn, "%Y-%m-%d %H:%M:%S")
        AccessLevelActivateOn = result + timedelta(days=730)
        return AccessLevelDeactivateOn.strftime("%Y-%m-%d %H:%M:%S")
    else:
        AccessLevelActivateOn = "PERMANENT"
        return AccessLevelDeactivateOn
...
def generate_profile():
    ...
    for _ in range(accessLvl_count):
        accessLvl.append({
            ...
            "AccessLevel_ActivateOn": generate_randdom_AccessLevelActivateOn(),
            "AccessLevel_Deactivate": generate_randdom_AccessLevelDeactivateOn()
        })
