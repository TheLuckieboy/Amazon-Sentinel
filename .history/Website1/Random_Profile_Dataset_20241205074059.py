...
def generate_random_badgeStatus():
    if EmployeeStatus == "Active":
        ...
        BadgeStatus = [status for i, status in enumerate(badge_status) if i not in exclude_indices]
    else:
        exclude_indices = [0]
        BadgeStatus = [status for i, status in enumerate(badge_status) if i not in exclude_indices]
...
def generate_random_badgeType():
    if EmployeeType == "Contractor":
        exclude_indices = [0, 1, 2, 3, 4, 5]
        BadgeType = [status for i, status in enumerate(badge_type) if i not in exclude_indices]
    elif EmployeeType == "Employee (Seasonal)":
        exclude_indices = [1, 2, 4, 5 , 6, 7]
        BadgeType = [status for i, status in enumerate(badge_type) if i not in exclude_indices]
    else:
        exclude_indices = [0, 3, 6, 7]
        BadgeType = [status for i, status in enumerate(badge_type) if i not in exclude_indices]
...
def generate_random_Date(BadgeActivation=False, BadgeDeactivation=False, LastUpdate=False):
    ...
    if BadgeActivation:
        ...
        DeactivationDate = datetime.strptime(ActivationDate, "%Y-%m-%d") + timedelta(days=730)   
        return DeactivationDate.strftime("%Y-%m-%d") + " 23:59:00" 

    elif LastUpdate:
        LastUpdateDate = datetime.strptime(ActivationDate, "%Y-%m-%d") + timedelta(days=random.randint(0, 30))
...
def generate_profile():
    return {
        ...
        "Badges_Count": generate_random_string(Just_Numbers=True, length=4),
        "Badge_ID": generate_random_string(Just_Numbers=True, length=8),
        "Badge_Status": generate_random_badgeStatus(),
        "Badge_Type": generate_random_badgeType(),
        "Badge_ActivateOn": gernerate_random_Date(BadgeActivation=True),
        "Badge_DeactivateOn": gernerate_random_Date(BadgeDeactivation=True),
        "Badge_LastUpdatedUTC": gernerate_random_Date(LastUpdate=True),
        "Badge_LastLocationReaderName": random.choice(badge_locationRead),
        "Badge_LastLocationEventType": "Access Granted",

        "Accesslevels_Count": generate_random_string(Just_Numbers=True, length=4),
        "AccessLevel": random.choice(badge_status),
