...
def generate_random_Date(BadgeActivation=False, BadgeDeactivation=False, LastUpdate=False, base_date=None, next_activation_date=None):
    ...
    if BadgeActivation:
        ...
        return ActivationDate.strftime("%Y-%m-%d %H:%M:%S")
        ...
        # Generate a random last update date between activation and the latest allowed date
        days_to_update = random.randint(1, 729)
        LastUpdateDate = ActivationDate + timedelta(days=days_to_update)

        # Ensure LastUpdate does not exceed the next_activation_date
        if next_activation_date:
            next_activation_datetime = datetime.strptime(next_activation_date, "%Y-%m-%d %H:%M:%S")
            if LastUpdateDate >= next_activation_datetime:
                LastUpdateDate = next_activation_datetime - timedelta(days=1)

        return LastUpdateDate.strftime("%Y-%m-%d %H:%M:%S")
...
def generate_profile():
    global Region, BuildingLocation, ActiveBadge, LocationInOrOut, GeneralAccess
    ...
    for _ in range(badge_count):
        # Generate badge activation date relative to latest allowed date
        badge_activate = generate_random_Date(BadgeActivation=True, base_date=latest_allowed_date)

        # Update the latest allowed date to ensure LastUpdated doesn't exceed this activation
        latest_allowed_date = datetime.strptime(badge_activate, "%Y-%m-%d %H:%M:%S")

        # Generate LastUpdate with restriction to not exceed the current latest allowed date
        badge_lastupdate = generate_random_Date(LastUpdate=True, next_activation_date=latest_allowed_date.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        latest_allowed_date -= timedelta(days=random.randint(30, 90))
