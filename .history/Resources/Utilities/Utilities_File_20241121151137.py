...
def Plugins(settings):
    ...
    NATACS_Terminate_AA_Plugin = False
    Plugins.append(NATACS_Terminate_AA_Plugin)
...
def Plugin_Widget_Setup(FunctionsGUI, widget, settings, Save_Widget_Settings, grid_layout, Script_Widgets, index1):
    if index1 == 0:
        try:
            from Resources.Scripts.Cardholder.Cardholder_Verification_File import Widget_Setup
...
def Script_Launcher(get_next_index, function_mapping, index1):
    if index1 == 0:
        try:
            from Resources.Scripts.Cardholder.Cardholder_Verification_File import Cardholder_Verification
        ...
        try:
            from Resources.Scripts.Skyline.Skyline_Terminate_AA_File import Skyline_Terminate_AA
            next_index = get_next_index(function_mapping)
...
def Extra_GUI(FunctionsGUI, settings, Save_Widget_Settings):
    ...
    Cardholder_Verification_Plugin = FunctionsGUI.Widget_Creator("Cardholder Verification", Font_Size=8, Checkmark=True)
    ...
    for widget in Widgets:
        ...
        if widget.objectName() == "Cardholder_Verification_Plugin":
            ...
            Cardholder_Verification_Plugin.checkmark.setChecked(settings.get("Cardholder_Verification_Plugin", False))
