...
def Import_File(File_Name, zip_path, Zip_Password="PASSWORD", File_Password="PASSWORD", temp_dir="./temp"):
    ...
    def decrypt_file(encrypted_path: Path, password: str) -> Path:
        ...
        try:
            with open(encrypted_path, 'rb') as encrypted_file:
                ...
                # Remove the `.enc` extension to get the original file name
    ...
    try:
        ...
        # Locate the encrypted file dynamically based on File_Name
        encrypted_file = None
        for file in Path(temp_dir).glob(f"{File_Name}.*.enc"):
            encrypted_file = file
            break

        if not encrypted_file:
            raise FileNotFoundError(f"Encrypted file for {File_Name} not found in {temp_dir}.")

        # Decrypt the file
        decrypted_path = decrypt_file(encrypted_file, File_Password)
...
utilities_file = Import_File("Utilities_File.py", "Resources/Test/Utilities/Utilities_File.py.zip")
