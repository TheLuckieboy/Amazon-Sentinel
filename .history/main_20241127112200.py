...
def Import_File(File_Name, zip_path, Zip_Password="PASSWORD", File_Password="PASSWORD", temp_dir="./temp"):
    ...
    def detect_file_type(file_path: Path) -> str:
        """Determine the file type based on the extension or content."""
        if file_path.suffix == ".py":
            return "python"
        elif file_path.suffix == ".txt":
            return "text"
        elif file_path.suffix == ".qss":
            return "qss"
        else:
    ...
    try:
        ...
        decrypted_path = Path(temp_dir) / File_Name
        decrypt_file(str(encrypted_path), str(decrypted_path), File_Password)

        # Detect the file type
        file_type = detect_file_type(decrypted_path)
