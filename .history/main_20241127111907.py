...
def Import_File(File_Name, zip_path, Zip_Password="PASSWORD", File_Password="PASSWORD", temp_dir="./temp"):
    ...
    try:
        ...
        encrypted_path = Path(temp_dir) / f"{File_Name}.enc"
        decrypted_path = encrypted_path.with_suffix(f".{file_extension}")

        # Decrypt the file
        decrypt_file(str(encrypted_path), str(decrypted_path), File_Password)

        # Return file path or content based on the type
        if file_extension.lower() in {"png", "ico", "jpg", "jpeg"}:
            # For binary files, return the file path
            print(f"Decrypted binary file: {decrypted_path}")
            return str(decrypted_path)
        else:
            # For text-based files, return the content
            with open(decrypted_path, "r", encoding="utf-8") as file:
                content = file.read()
            print(f"Decrypted text file: {decrypted_path}")
            return content
    except Exception as e:
        raise RuntimeError(f"File import failed: {e}")
    finally:
