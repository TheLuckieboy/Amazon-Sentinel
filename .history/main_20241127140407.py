...
class BasePage(QMainWindow):
    ...
    def setup_corner_images(self):
        # Labels for the corner images
        self.bottom_left_label = QLabel(self)
        self.bottom_right_label = QLabel(self)

        # Define the names of the images
        bottom_left_image_name = "bottom_left_corner"
        bottom_right_image_name = "bottom_right_corner"

        # Path to the ZIP file containing the images
        zip_path = resource_path(os.path.join("Resources", "Test", "Utilities", "Images.zip"))

        # Extract and load the images
        try:
            bottom_left_pixmap = Import_File(bottom_left_image_name, zip_path, password="PASSWORD")
            bottom_right_pixmap = Import_File(bottom_right_image_name, zip_path, password="PASSWORD")

            if isinstance(bottom_left_pixmap, QPixmap) and isinstance(bottom_right_pixmap, QPixmap):
                # Set the pixmaps to the labels
                self.bottom_left_label.setPixmap(bottom_left_pixmap)
                self.bottom_right_label.setPixmap(bottom_right_pixmap)
                ...
                raise ValueError("Failed to load one or both images as QPixmap objects.")
        except Exception as e:
            print(f"Error setting up corner images: {e}")
