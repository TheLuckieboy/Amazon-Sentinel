...
class CachedModules:
    ...
    @classmethod
    def get_module(cls, file_path, module_name):
...
def process_locked_scripts(zip_path: str, password: str, temp_dir: str):
    # Step 1: Extract ZIP
    extract_zip(zip_path, temp_dir, password)
    
    # Step 2: Decrypt Files
    for file in Path(temp_dir).glob("*.enc"):
        decrypted_path = file.with_suffix('.py')
        decrypt_file(str(file), str(decrypted_path), password)
        
        # Step 3: Load Decrypted Python File
        load_python_file(str(decrypted_path))
        
        # Optional: Delete the decrypted file after loading for security
        os.remove(decrypted_path)
    print("Processing complete.")

def dynamic_import(decrypted_file_path: str, module_name: str):
    """
    Dynamically import a Python file as a module.
    """
    spec = importlib.util.spec_from_file_location(module_name, decrypted_file_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module

def load_encrypted_module(zip_path, password, temp_dir, module_name):
    """
    Extracts, decrypts, and dynamically loads an encrypted Python module.
    
    Args:
        zip_path (str): Path to the ZIP file containing the encrypted Python file.
        password (str): Password to unlock the ZIP and decrypt the file.
        temp_dir (str): Temporary directory to store extracted files.
    ...
    # Locate and decrypt the encrypted Python file
    encrypted_path = Path(temp_dir) / f"{module_name}.py.enc"
    decrypted_path = encrypted_path.with_suffix(".py")
    decrypt_file(str(encrypted_path), str(decrypted_path), password)

    # Dynamically import the module
    spec = importlib.util.spec_from_file_location(module_name, str(decrypted_path))
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)

    # Cleanup: Delete the decrypted file
    os.remove(decrypted_path)
...
class BasePage(QMainWindow):
    ...
    def setup_corner_images(self):
        ...
        bottom_left_image_path = resource_path(os.path.join("Resources", "Images", "Border", "bottom_left_corner.png"))
        bottom_right_image_path = resource_path(os.path.join("Resources", "Images", "Border", "bottom_right_corner.png"))
...
class FunctionsGUI(BasePage):
    ...
    """def Plugin_Settings(self):
        from Resources.Utilities.Utilities_File import Plugins
        self.Plugins = Plugins(settings)
        self.Length_Plugins = len(self.Plugins) // 3"""

    def Plugin_Settings(self):
        # Define paths and credentials
        zip_path = "Resources/Test/Utilities/Utilities_File.py.zip"
        password = "PASSWORD"
        temp_dir = "./temp"
        module_name = "Utilities_File"

        # Load the encrypted module
        utilities_module = load_encrypted_module(zip_path, password, temp_dir, module_name)

        # Access the Plugins function dynamically
        Plugins = getattr(utilities_module, "Plugins", None)
        if not Plugins:
    ...
    def Script_Specific_Settings(self):
        ...
        for _ in range(self.Length_Plugins):
            if self.Plugins[index1]:
                from Resources.Utilities.Utilities_File import Plugin_Widget_Setup
                widget = QWidget()
    ...
    def Widget_Creator(self, label_text="Add Label", label_text2="Add Label", ButtonLabel="Add Label",
        ...
        if Column_Letter_Box and Quip_Color_Box:
            ...
            try:
                # Define the path to Site_Locations.txt
                site_location_path = resource_path(os.path.join("Resources", "Site_Locations.txt"))

                # Open and read the file
                with open(site_location_path, 'r') as file:
                    Site_Location_List = [line.strip() for line in file.readlines()]

            except FileNotFoundError:
                Site_Location_List = []
                print("Site_Locations.txt file not found.")
    ...
    def Open_Browser(self, Chrome=False, Firefox=False, Edge=False):
        if not self.driver_is_Open:
            from Resources.Utilities.Utilities_File import Open_Browser
    ...
    def run_selected_function(self):
        ...
        for _ in range(self.Length_Plugins):
            if self.Plugins[index1]:
                from Resources.Utilities.Utilities_File import Script_Launcher
...
class HelpGUI(BasePage):
    ...
    def HelpGUI_Widget(self, page1, settings, Save_Widget_Settings):
        from Resources.Utilities.Utilities_File import Help_GUI
...
class ExtraGUI(BasePage):
    ...
    def ExtraGUI_Widget(self, page1, settings, Save_Widget_Settings):
        from Resources.Utilities.Utilities_File import Extra_GUI
