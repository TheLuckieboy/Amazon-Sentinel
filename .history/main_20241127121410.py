...
"""def Import_File(File_Name, zip_path, Python_File=False, Text_File=False, QSS_File=False, 
                Zip_Password="PASSWORD", File_Password="PASSWORD", temp_dir="./temp"):
    import zipfile, importlib.util, shutil, os, sys
    ...
    def extract_zip(zip_path: str, extract_to: str, password: str):
        try:
    ...
    def decrypt_file(encrypted_path: str, output_path: str, password: str):
        ...
        try:
            with open(encrypted_path, 'rb') as encrypted_file:
                ...
                if len(content) < 32:
                    raise ValueError("Encrypted file is too small to contain necessary metadata.")
    ...
    if sum([Python_File, Text_File, QSS_File]) != 1:
        raise ValueError("Specify exactly one file type: Python_File, Text_File, or QSS_File.")
    ...
    try:
        ...
        extract_zip(zip_path, temp_dir, Zip_Password)

        # Determine file extension
        file_extension = "py" if Python_File else "txt" if Text_File else "qss"

        # Locate and decrypt the file
        encrypted_path = Path(temp_dir) / f"{File_Name}.{file_extension}.enc"
        decrypted_path = encrypted_path.with_suffix(f".{file_extension}")
        decrypt_file(str(encrypted_path), str(decrypted_path), File_Password)
...
def decrypt_file(encrypted_path: Path, output_folder: Path, password: str):
    try:
        with open(encrypted_path, 'rb') as enc_file:
            ...
            output_file = output_folder / encrypted_path.name
...
def Import_File(File_Name: str, zip_path: str, password = "PASSWORD", temp_dir="./temp"):
    """
    Import a file (Python, text, image, etc.) from a zip archive after decryption.
    
    :param File_Name: Name of the file (without extension) inside the zip archive.
    :param zip_path: Path to the zip file containing the encrypted file.
    :param password: Password for decrypting the files.
    :param temp_dir: Temporary directory to extract and decrypt files.
    :return: The imported file (module, content, or QPixmap)
    """
    try:
        os.makedirs(temp_dir, exist_ok=True)
        ...
        # Locate the encrypted file and decrypt it
        encrypted_path = extract_folder / f"{File_Name}.py.enc"
        ...
        if file_extension == ".py":
            # For Python files, import them as modules
            spec = importlib.util.spec_from_file_location(File_Name, str(decrypted_path))
            Imported_File = importlib.util.module_from_spec(spec)
            sys.modules[File_Name] = Imported_File
            spec.loader.exec_module(Imported_File)
            return Imported_File
        elif file_extension == ".txt" or file_extension == ".qss":
            # For text and QSS files, return the content as a string
            with open(decrypted_path, "r", encoding="utf-8") as file:
                return file.read()
        elif file_extension in [".png", ".ico"]:
            # For image files, return as QPixmap object for PyQt
...
utilities_file = Import_File("Utilities_File", "Resources/Test/Utilities/Utilities_File.py.zip")

# Access class and function from the imported file
StopFunctionException = getattr(utilities_file, "StopFunctionException", None)
check_stop_event = getattr(utilities_file, "check_stop_event", None)

# You can now use them as normal
if StopFunctionException:
    raise StopFunctionException("Stopping function execution.")
if check_stop_event:
    result = check_stop_event()
    print(result)
...
class BasePage(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(resource_path(os.path.join("Resources", "Images", "Icon.png"))))
    ...
    def setup_sidebar_buttons(self):
        icons = [
            (resource_path(os.path.join("Resources", "Images", "BurgerMenuIcon.png")), "BurgerMenuButton"),
            (resource_path(os.path.join("Resources", "Images", "FunctionsIcon.png")), "functionsButton", 0),
            (resource_path(os.path.join("Resources", "Images", "HelpIcon.png")), "helpButton", 1),
            (resource_path(os.path.join("Resources", "Images", "ExtraIcon.png")), "extraButton", 2),
        ]

        icons_with_desc = [
            (resource_path(os.path.join("Resources", "Images", "BurgerMenuIconWithDescription.png")), "BurgerMenuButton"),
            (resource_path(os.path.join("Resources", "Images", "FunctionsIconWithDescription.png")), "functionsButton", 0),
            (resource_path(os.path.join("Resources", "Images", "HelpIconWithDescription.png")), "helpButton", 1),
            (resource_path(os.path.join("Resources", "Images", "ExtraIconWithDescription.png")), "extraButton", 2),
        ]
    ...
    def setup_corner_images(self):
        ...
        bottom_left_image_path = resource_path(os.path.join("Resources", "Images", "bottom_left_corner.png"))
        bottom_right_image_path = resource_path(os.path.join("Resources", "Images", "bottom_right_corner.png"))
...
class FunctionsGUI(BasePage):
    ...
    def Plugin_Settings(self):
        ...
        Plugins = getattr(utilities_file, "Plugins", None)
        if not Plugins:
            raise ImportError("The function 'Plugins' was not found in Utilities_File.")

        # Use the Plugins function
        self.Plugins = Plugins(settings)
        self.Length_Plugins = len(self.Plugins) // 3
    ...
    def Execution_Output(self):
        ...
        def Function_Section():
            ...
            def Confirm_Function_Image():
                ...
                red_pixmap = QPixmap(resource_path(os.path.join("Resources", "Images", "ConfirmedRed.png")))
                ...
                green_pixmap = QPixmap(resource_path(os.path.join("Resources", "Images", "ConfirmedGreen.png")))
    ...
    def Widget_Creator(self, label_text="Add Label", label_text2="Add Label", ButtonLabel="Add Label",
        ...
        if Column_Letter_Box and Quip_Color_Box:
            ...
            try:
                ...
                Site_Location_List_Content = Import_File("Site_Locations", "Resources/Test/Utilities/Site_Locations.txt.zip")
    ...
    def Open_Browser(self, Chrome=False, Firefox=False, Edge=False):
        ...
        if not self.driver_is_Open:
            self.driver, self.window_handles, self.driver_is_Open = Open_Browser(self, self.Plugins, self.function_combobox, Chrome, Firefox, Edge)
    ...
    def check_driver_status(self):
        while True:
            try:
                DriverIsPresentlyOpen = self.driver.find_elements(By.CSS_SELECTOR, "head")
            except:
                try:
                    time.sleep(3)
                    DriverIsPresentlyOpen = self.driver.find_elements(By.CSS_SELECTOR, "head")
    ...
    def run_selected_function(self):
        ...
        selected_function_index = self.function_combobox.currentIndex()
        ...
        thread = threading.Thread(target=self.run_function_with_kill_switch, args=(selected_function,))
        thread.start()

    def run_function_with_kill_switch(self, func):
        # Define a stop event
        ...
        if DriverDoesExist is not None:
            ...
            while not stop_event.is_set():
                if not func(self.driver, self.window_handles, WorkingRow, settings=settings, stop_event=stop_event):
...
if __name__ == "__main__":
    ...
    try:
        ...
        stylesheet = Import_File("styles", "Resources/Test/Utilities/styles.qss.zip")
        if stylesheet:
            app.setStyleSheet(stylesheet)
    except FileNotFoundError:
