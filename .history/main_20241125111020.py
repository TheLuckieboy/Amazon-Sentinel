import pyautogui, threading, keyboard, time, json, string, pyperclip, logging, datetime, os, sys, zipfile, importlib.util
...
gtime = 0.25
zip_path = "C:/Users/wmichluc/Documents/Projects/Programming/Python/Offical Release/Sentinel V0.0.2/Resources/Test/Utilities/Utilities_File.py.zip"
...
class FunctionsGUI(BasePage):
    def __init__(self, parent=None):
        ...
        self.Plugin_Settings(zip_path, "PASSWORD", "./temp")
    ...
    def Plugin_Settings(self, zip_path: str, password: str, temp_dir: str):
        # Extract, decrypt, and load the module
        extract_zip(zip_path, temp_dir, password)
        
        # Decrypt the Utilities_File.py.enc
        encrypted_path = Path(temp_dir) / "Utilities_File.py.enc"
        decrypted_path = encrypted_path.with_suffix(".py")
        decrypt_file(str(encrypted_path), str(decrypted_path), password)
        
        # Load the module dynamically or fetch it from cache
        module_name = "Utilities_File"
        utilities_module = CachedModules.get_module(str(decrypted_path), module_name)

        # Access the Plugins function from the module
